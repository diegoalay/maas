class Service < ApplicationRecord
  ...
  has_many :working_weeks, dependent: :destroy
  ...
  def available_shifts
    ...
    # search or create shift
    return (week_init..(5+week_init)).map { |n| { shift_id: WorkingWeek.create_shift(self, n).id,  text: "Semana #{n}", value: n }}
  end
  ...
  def self.index
    services = Service.select(
      ...
      'services.created_at',
      'services.name',
      "trim(concat(users.first_name, ' ', users.last_name)) as user_creator_name"
    )
    .joins(:user_creator)
  end
end